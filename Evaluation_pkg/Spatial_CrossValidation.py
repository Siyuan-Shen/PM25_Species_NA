...
from Evaluation_pkg.data_func import Get_SecondStage_XIndex,GetXIndex,GetYIndex,Get_XY_indices, Get_XY_arraies, Get_final_output, ForcedSlopeUnity_Func, CalculateAnnualR2, CalculateMonthR2, calculate_Statistics_results
from Evaluation_pkg.iostream import load_coMonitor_Population,save_SecondStage_trained_model,save_trained_model, output_text, save_loss_accuracy, save_data_recording, load_data_recording, AVD_output_text
...
def AVD_Spatial_CrossValidation(width, height, sitesnumber,start_YYYY, TrainingDatasets):
    ...
    for train_index, test_index in rkf.split(site_index):
        for imodel in range(len(beginyears)):
            print('imodel is: {}'.format(beginyears[imodel]))
