...
from Uncertainty_pkg.iostream import load_NA_GeoLatLon,load_NA_GeoLatLon_Map
...
def convert_distance_to_rRMSE_uncertainty(buffer_radii, BLOO_rRMSE, map_distances,):
    ...
    map_uncertainty[outrange_pixels_index] =  (map_distances[outrange_pixels_index]-d_left)(d_right-d_left) * (rRMSE_right - rRMSE_left) +rRMSE_left
    return map_uncertainty

def get_nearest_site_distance_for_each_pixel():
    SATLAT,SATLON = load_NA_GeoLatLon()
    tSATLAT_map,tSATLON_map = load_NA_GeoLatLon_Map()
    ...
    nearest_lat_map = interp_lat(tSATLAT_map,tSATLON_map)
    nearest_lon_map = interp_lon(tSATLAT_map,tSATLON_map)
    ...
    nearest_distance_map = np.full(nearest_lat_map.shape,1000)
    for ix in range(len(SATLAT)):
        land_index = np.where(landtype[ix,:] != 0)
        print('It is procceding ' + str(np.round(100*(ix/len(SATLAT)),2))+'%.' )
        if len(land_index[0]) == 0:
            ...
            nearest_distance_map[ix,land_index[0]] = calculate_distance_forArray(nearest_lat_map[ix,land_index[0]],
                                                                        ...
                                                                        tSATLAT_map[ix,land_index[0]],tSATLON_map[ix,land_index[0]])
            end_time = time.time()
    ...
    outdir = '/my-projects/PM25_Speices_DL_2023/data/Pixels2sites_distances/'
    if not os.path.isdir(outdir):
        os.makedirs(outdir)
    outfile = outdir + '{}_nearest_site_distances_forEachPixel.npy'.format(species)
    np.save(outfile,nearest_distance_map)
    return 
