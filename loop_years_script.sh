...
for year in $(seq $start_year $end_year); do
    ...
    sed -i "s/^#BSUB -J .*/#BSUB -J \"1.7.2 PM ${year}\"/" $modified_script

    # Update the pause_time calculation
    sed -i "s/^pause_time=\$((RANDOM % 50 .*/pause_time=\$((RANDOM % 10 + (${year} - ${start_year}) * 120))/" $modified_script
    ...
    echo "Waiting for 5 seconds before the next job..."
    sleep 5
