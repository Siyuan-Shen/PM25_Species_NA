...
def Train_Model_forEstimation(train_beginyears, train_endyears, training_months,width, height, sitesnumber,start_YYYY, TrainingDatasets,total_channel_names,main_stream_channel_names, side_stream_nchannel_names):
    ...
    for imodel_year in range(len(train_beginyears)):
        ...
        for imodel_month in range(len(training_months)):
            ...
            learning_objective_index   = Get_month_based_Index(index=site_index,model_beginyear=1998,beginyear=train_beginyears[imodel_year],endyear=train_endyears[imodel_year],month_index=training_months[imodel_month],sitenumber=sitesnumber)
            ...
            teating_objective_index    = Get_month_based_Index(index=np.array(range(100)),model_beginyear=1998,beginyear=train_beginyears[imodel_year],endyear=train_endyears[imodel_year],month_index=training_months[imodel_month],sitenumber=sitesnumber) #
...
def Original_Train_Model_forEstimation(train_beginyears, train_endyears, width, height, sitesnumber,start_YYYY, TrainingDatasets,total_channel_names,main_stream_channel_names, side_stream_nchannel_names):
    ...
    for imodel in range(len(train_beginyears)):
        Normalized_TrainingData    = get_trainingdata_within_start_end_YEAR(initial_array=Initial_Normalized_TrainingData, training_start_YYYY=train_beginyears[imodel],training_end_YYYY=train_endyears[imodel],start_YYYY=start_YYYY,sitesnumber=sitesnumber)
